# Slow Query Weekly Increase - WoW Spike Alert
...
|----------------|----------|
| **Alert ID** | ALR-135 |
| **告警名称 Alert Name** | Slow Query Weekly Increase - WoW Spike Alert |
| **优先级 Priority** | P2 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | Database-MySQL |
| **系统 System** | Grafana |
| **指标类型 Metric Type** | Performance |
| **阈值/条件 Threshold** | weekly increase > 10000 |
| **持续时间 Duration** | 1h |
...
### 中文说明
此告警在 **weekly increase > 10000** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L1** 级别服务。

**触发条件:** Performance 指标达到阈值 weekly increase > 10000，持续时间: 1h

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **weekly increase > 10000** is met. This is a **P2** priority alert affecting **L1** level services.

**Trigger Condition:** Performance metric reaches threshold weekly increase > 10000, duration: 1h
...
|-----------------|------------|------------------|-----------|
| All MySQL RDS | L0 | 所有MySQL RDS (59 instances) | DBA |

### 关联数据库 Affected Databases

- `All luckyus_* MySQL databases (59 instances)`
...
|---------------|--------------------------|---------------------------|
| 初次响应无法解决 / Initial response cannot resolve | 1h | L2 Support |
...
```
1. 告警名称和ID / Alert name and ID: Slow Query Weekly Increase - WoW Spike Alert (ALR-135)
