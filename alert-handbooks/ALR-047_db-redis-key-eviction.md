...
|----------------|----------|
| **Alert ID** | ALR-047 |
| **告警名称 Alert Name** | 【DB告警】Redis 实例触发key淘汰 |
| **优先级 Priority** | P2 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | Database-Redis |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Eviction |
| **阈值/条件 Threshold** | Key eviction triggered |
...
### 中文说明
此告警在 **Key eviction triggered** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L1** 级别服务。

**触发条件:** Eviction 指标达到阈值 Key eviction triggered，持续时间: Instant

**重要性:** 此告警关联 存储平台 团队负责的 Redis Cloud, LDAS 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **Key eviction triggered** is met. This is a **P2** priority alert affecting **L1** level services.

**Trigger Condition:** Eviction metric reaches threshold Key eviction triggered, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】Redis 实例触发key淘汰 (ALR-047)
