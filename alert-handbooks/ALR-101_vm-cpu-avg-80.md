...
### 中文说明
此告警在 **>80% avg usage** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** CPU 指标达到阈值 >80% avg usage，持续时间: Instant

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **>80% avg usage** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** CPU metric reaches threshold >80% avg usage, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【vm-CPU】P1 服务整体CPU平均使用率超过80% (ALR-101)
