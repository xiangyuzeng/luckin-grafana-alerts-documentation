...
|----------------|----------|
| **Alert ID** | ALR-110 |
| **告警名称 Alert Name** | 【vm-宕机】P0 up监控指标心跳丢失10分钟需检查设备是否宕机 |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | VM |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Availability |
| **阈值/条件 Threshold** | Heartbeat lost for 10min |
| **持续时间 Duration** | 10m |
...
### 中文说明
此告警在 **Heartbeat lost for 10min** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Availability 指标达到阈值 Heartbeat lost for 10min，持续时间: 10m

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **Heartbeat lost for 10min** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Availability metric reaches threshold Heartbeat lost for 10min, duration: 10m
...
```
1. 告警名称和ID / Alert name and ID: 【vm-宕机】P0 up监控指标心跳丢失10分钟需检查设备是否宕机 (ALR-110)
