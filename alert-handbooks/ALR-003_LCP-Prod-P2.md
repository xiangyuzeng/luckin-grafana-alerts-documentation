...
|----------------|----------|
| **Alert ID** | ALR-003 |
| **告警名称 Alert Name** | [LCP-Prod-P2] |
| **优先级 Priority** | P2 |
| **服务等级 Service Level** | L2 - Normal Business/Infrastructure (普通业务服务/普通基础服务) |
| **类别 Category** | Priority Levels |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Multi-condition |
| **阈值/条件 Threshold** | LCP Production Medium Aggregator |
| **持续时间 Duration** | N/A |
...
### 中文说明
此告警在 **LCP Production Medium Aggregator** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L2** 级别服务。

**触发条件:** Multi-condition 指标达到阈值 LCP Production Medium Aggregator，持续时间: N/A

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **LCP Production Medium Aggregator** is met. This is a **P2** priority alert affecting **L2** level services.

**Trigger Condition:** Multi-condition metric reaches threshold LCP Production Medium Aggregator, duration: N/A

**Importance:** This alert is associated with the SRE team's Infrastructure, Networking domain services and requires response within < 30 minutes.
...
|-----------------|------------|------------------|-----------|
| All Services | L0-L2 | 所有服务 (Priority Aggregator) | SRE |
...
```
1. 告警名称和ID / Alert name and ID: [LCP-Prod-P2] (ALR-003)
