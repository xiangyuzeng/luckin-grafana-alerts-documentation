...
|----------------|----------|
| **Alert ID** | ALR-085 |
| **告警名称 Alert Name** | 【默认策略】FGC次数大于0或YGC耗时大于500毫秒 |
| **优先级 Priority** | P2 |
| **服务等级 Service Level** | L2 - Normal Business/Infrastructure (普通业务服务/普通基础服务) |
| **类别 Category** | APM-Default |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | JVM |
| **阈值/条件 Threshold** | FGC >0 or YGC >500ms |
...
### 中文说明
此告警在 **FGC >0 or YGC >500ms** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L2** 级别服务。

**触发条件:** JVM 指标达到阈值 FGC >0 or YGC >500ms，持续时间: Instant

**重要性:** 此告警关联 运维开发 团队负责的 Auth, CI/CD, Monitoring 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **FGC >0 or YGC >500ms** is met. This is a **P2** priority alert affecting **L2** level services.

**Trigger Condition:** JVM metric reaches threshold FGC >0 or YGC >500ms, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【默认策略】FGC次数大于0或YGC耗时大于500毫秒 (ALR-085)
