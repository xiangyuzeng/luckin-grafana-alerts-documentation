...
### 中文说明
此告警在 **NIC status down** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Network 指标达到阈值 NIC status down，持续时间: Instant

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **NIC status down** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Network metric reaches threshold NIC status down, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【vm-网卡】P0 网卡状态为down (ALR-116)
