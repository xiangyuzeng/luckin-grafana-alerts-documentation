# 【iZeus】Transfer-OAPTrace2Receiver-0
...
|----------------|----------|
| **Alert ID** | ALR-084 |
| **告警名称 Alert Name** | 【iZeus】Transfer-OAPTrace2Receiver-0 |
| **优先级 Priority** | P1 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | APM-iZeus |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Transfer |
| **阈值/条件 Threshold** | OAP Trace to Receiver Issue |
...
### 中文说明
此告警在 **OAP Trace to Receiver Issue** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** Transfer 指标达到阈值 OAP Trace to Receiver Issue，持续时间: Instant

**重要性:** 此告警关联 监控平台 团队负责的 iZeus APM, Grafana 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **OAP Trace to Receiver Issue** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** Transfer metric reaches threshold OAP Trace to Receiver Issue, duration: Instant

**Importance:** This alert is associated with the Monitoring team's iZeus APM, Grafana domain services and requires response within < 15 minutes.
...
```
1. 告警名称和ID / Alert name and ID: 【iZeus】Transfer-OAPTrace2Receiver-0 (ALR-084)
