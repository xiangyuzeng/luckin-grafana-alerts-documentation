...
### 中文说明
此告警在 **<10GB (Voice)** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Disk 指标达到阈值 <10GB (Voice)，持续时间: Instant

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **<10GB (Voice)** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Disk metric reaches threshold <10GB (Voice), duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】AWS-ES磁盘空间不足10G_语音 (ALR-039)
