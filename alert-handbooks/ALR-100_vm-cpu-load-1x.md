...
|----------------|----------|
| **Alert ID** | ALR-100 |
| **告警名称 Alert Name** | 【vm-CPU】P1 CPU平均负载大于CPU核心数量的1倍已持续5分钟 |
| **优先级 Priority** | P1 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | VM |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | CPU |
| **阈值/条件 Threshold** | Load avg >1x cores for 5min |
| **持续时间 Duration** | 5m |
...
### 中文说明
此告警在 **Load avg >1x cores for 5min** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** CPU 指标达到阈值 Load avg >1x cores for 5min，持续时间: 5m

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **Load avg >1x cores for 5min** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** CPU metric reaches threshold Load avg >1x cores for 5min, duration: 5m
...
1. 进程CPU使用率过高 / Process CPU usage too high
2. 系统中断或软中断过多 / Excessive system interrupts or soft interrupts
3. I/O等待导致CPU占用 / I/O wait causing CPU occupation
4. CPU窃取(虚拟化环境) / CPU steal (virtualization environment)
5. 僵尸进程占用资源 / Zombie processes consuming resources
...
```
1. 告警名称和ID / Alert name and ID: 【vm-CPU】P1 CPU平均负载大于CPU核心数量的1倍已持续5分钟 (ALR-100)
