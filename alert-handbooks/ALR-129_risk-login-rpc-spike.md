...
|----------------|----------|
| **Alert ID** | ALR-129 |
| **告警名称 Alert Name** | 【国际化北美风控】登录rpc接口最近5分钟调用总量超过100次且比上周同时段多60% |
| **优先级 Priority** | P1 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | Risk Control |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Traffic |
| **阈值/条件 Threshold** | Login RPC >100 calls +60% WoW |
| **持续时间 Duration** | 5m |
...
### 中文说明
此告警在 **Login RPC >100 calls +60% WoW** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L0** 级别服务。

**触发条件:** Traffic 指标达到阈值 Login RPC >100 calls +60% WoW，持续时间: 5m

**重要性:** 此告警关联 风控 团队负责的 Fraud Prevention 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **Login RPC >100 calls +60% WoW** is met. This is a **P1** priority alert affecting **L0** level services.

**Trigger Condition:** Traffic metric reaches threshold Login RPC >100 calls +60% WoW, duration: 5m
...
```
1. 告警名称和ID / Alert name and ID: 【国际化北美风控】登录rpc接口最近5分钟调用总量超过100次且比上周同时段多60% (ALR-129)
