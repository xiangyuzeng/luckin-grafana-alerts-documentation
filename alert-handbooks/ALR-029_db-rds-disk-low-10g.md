...
### 中文说明
此告警在 **<10GB for 3min** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L0** 级别服务。

**触发条件:** Disk 指标达到阈值 <10GB for 3min，持续时间: 3m

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **<10GB for 3min** is met. This is a **P1** priority alert affecting **L0** level services.

**Trigger Condition:** Disk metric reaches threshold <10GB for 3min, duration: 3m
...
1. 日志文件(binlog/慢查询日志)积累 / Log file accumulation (binlog/slow query logs)
2. 大型事务产生过多临时数据 / Large transactions generating excessive temporary data
3. 表空间碎片化 / Tablespace fragmentation
4. 未清理的备份或快照占用空间 / Uncleaned backups or snapshots consuming space
5. 数据增长超出预期 / Data growth exceeding expectations
...
**步骤 1:** 检查数据库大小分布 / Check database size distribution

**步骤 2:** 清理不需要的binlog / Purge unnecessary binlog: PURGE BINARY LOGS BEFORE 'date'

**步骤 3:** 清理慢查询日志 / Clean slow query logs

**步骤 4:** 检查并清理大表 / Check and clean large tables

**步骤 5:** 扩展RDS存储空间 / Extend RDS storage (AWS Console)
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】AWS RDS 磁盘空间连续3分钟不足10G (ALR-029)
