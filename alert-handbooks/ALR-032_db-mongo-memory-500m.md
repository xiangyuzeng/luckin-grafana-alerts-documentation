...
|----------------|----------|
| **Alert ID** | ALR-032 |
| **告警名称 Alert Name** | 【DB告警】AWS Mongo 可用内存连续三分钟不足500M |
| **优先级 Priority** | P1 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | Database-Mongo |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Memory |
| **阈值/条件 Threshold** | <500M for 3min |
| **持续时间 Duration** | 3m |
...
### 中文说明
此告警在 **<500M for 3min** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L0** 级别服务。

**触发条件:** Memory 指标达到阈值 <500M for 3min，持续时间: 3m

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **<500M for 3min** is met. This is a **P1** priority alert affecting **L0** level services.

**Trigger Condition:** Memory metric reaches threshold <500M for 3min, duration: 3m
...
- `luckyus_iriskcontrolservice (MongoDB/DocumentDB)`
...
```bash
# 检查DocumentDB集群状态 Check DocumentDB cluster status
aws docdb describe-db-clusters \
  --query 'DBClusters[*].{ID:DBClusterIdentifier,Status:Status}'

# MongoDB诊断命令 MongoDB diagnostic commands
mongo --host [DOCDB_ENDPOINT] --username admin --password [PASSWORD] --authenticationDatabase admin

# 在mongo shell中执行 Execute in mongo shell
db.currentOp()                          # 当前操作 Current operations
db.serverStatus()                       # 服务器状态 Server status
db.collection.stats()                   # 集合统计 Collection stats
db.collection.aggregate([{$indexStats:{}}])  # 索引使用 Index usage
```
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】AWS Mongo 可用内存连续三分钟不足500M (ALR-032)
