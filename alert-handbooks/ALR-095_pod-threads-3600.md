...
|----------------|----------|
| **Alert ID** | ALR-095 |
| **告警名称 Alert Name** | 【pod-线程】P0 容器线程数连续3分钟超过3600 |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | Pod |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Threads |
| **阈值/条件 Threshold** | >3600 threads for 3min |
| **持续时间 Duration** | 3m |
...
### 中文说明
此告警在 **>3600 threads for 3min** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Threads 指标达到阈值 >3600 threads for 3min，持续时间: 3m

**重要性:** 此告警关联 运维开发 团队负责的 Auth, CI/CD, Monitoring 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **>3600 threads for 3min** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Threads metric reaches threshold >3600 threads for 3min, duration: 3m
...
```
1. 告警名称和ID / Alert name and ID: 【pod-线程】P0 容器线程数连续3分钟超过3600 (ALR-095)
