...
### 中文说明
此告警在 **Global policy break** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Circuit Breaker 指标达到阈值 Global policy break，持续时间: Instant

**重要性:** 此告警关联 风控 团队负责的 Fraud Prevention 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **Global policy break** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Circuit Breaker metric reaches threshold Global policy break, duration: Instant

**Importance:** This alert is associated with the Risk Control team's Fraud Prevention domain services and requires response within < 5 minutes.
...
```
1. 告警名称和ID / Alert name and ID: 【北美亚风控】全局策略熔断告警 (ALR-123)
