...
|----------------|----------|
| **Alert ID** | ALR-027 |
| **告警名称 Alert Name** | 【DB告警】AWS RDS 活跃线程持续两分钟大于24 |
| **优先级 Priority** | P2 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | Database-RDS |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Performance |
| **阈值/条件 Threshold** | >24 active threads for 2min |
| **持续时间 Duration** | 2m |
...
### 中文说明
此告警在 **>24 active threads for 2min** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L1** 级别服务。

**触发条件:** Performance 指标达到阈值 >24 active threads for 2min，持续时间: 2m

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **>24 active threads for 2min** is met. This is a **P2** priority alert affecting **L1** level services.

**Trigger Condition:** Performance metric reaches threshold >24 active threads for 2min, duration: 2m
...
1. 缺少或过期的查询优化统计信息 / Missing or stale query optimization statistics
2. 表数据量增长导致查询变慢 / Table data growth causing query slowdown
3. 应用程序生成了低效的SQL / Application generating inefficient SQL
4. 临时表或排序操作溢出到磁盘 / Temporary tables or sort operations spilling to disk
5. 并发长时间运行的事务 / Concurrent long-running transactions
...
|---------------|--------------------------|---------------------------|
| 初次响应无法解决 / Initial response cannot resolve | 2m | L2 Support |
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】AWS RDS 活跃线程持续两分钟大于24 (ALR-027)
