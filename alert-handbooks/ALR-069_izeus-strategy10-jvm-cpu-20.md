...
### 中文说明
此告警在 **CPU >20%** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L2** 级别服务。

**触发条件:** JVM 指标达到阈值 CPU >20%，持续时间: Instant

**重要性:** 此告警关联 监控平台 团队负责的 iZeus APM, Grafana 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **CPU >20%** is met. This is a **P2** priority alert affecting **L2** level services.

**Trigger Condition:** JVM metric reaches threshold CPU >20%, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【iZeus-策略10】-JVM CPU使用率大于20 (ALR-069)
