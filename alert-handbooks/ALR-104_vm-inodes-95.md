...
|----------------|----------|
| **Alert ID** | ALR-104 |
| **告警名称 Alert Name** | 【vm-fileSystem】P0 分区inodes使用率大于95%请立即处理 |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | VM |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Disk |
| **阈值/条件 Threshold** | >95% inodes |
...
### 中文说明
此告警在 **>95% inodes** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Disk 指标达到阈值 >95% inodes，持续时间: Instant

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **>95% inodes** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Disk metric reaches threshold >95% inodes, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【vm-fileSystem】P0 分区inodes使用率大于95%请立即处理 (ALR-104)
