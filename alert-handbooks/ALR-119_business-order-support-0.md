...
### 中文说明
此告警在 **<1 order support for 10min** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Orders 指标达到阈值 <1 order support for 10min，持续时间: 10m

**重要性:** 此告警关联 销售业务 团队负责的 Orders, Payments, CRM, Marketing 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **<1 order support for 10min** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Orders metric reaches threshold <1 order support for 10min, duration: 10m
...
```
1. 告警名称和ID / Alert name and ID: 【北美-业务告警】订单支持持续10分钟小于1 (ALR-119)
