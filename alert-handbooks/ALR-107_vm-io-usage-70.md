...
|----------------|----------|
| **Alert ID** | ALR-107 |
| **告警名称 Alert Name** | 【vm-io】P1 磁盘IO使用率大于70%且同比超过20 |
| **优先级 Priority** | P1 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | VM |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | IO |
| **阈值/条件 Threshold** | >70% disk IO usage |
...
### 中文说明
此告警在 **>70% disk IO usage** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** IO 指标达到阈值 >70% disk IO usage，持续时间: Instant

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **>70% disk IO usage** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** IO metric reaches threshold >70% disk IO usage, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【vm-io】P1 磁盘IO使用率大于70%且同比超过20 (ALR-107)
