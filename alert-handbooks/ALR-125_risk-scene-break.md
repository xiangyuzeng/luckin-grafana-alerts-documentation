...
### 中文说明
此告警在 **Scene break** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Circuit Breaker 指标达到阈值 Scene break，持续时间: Instant

**重要性:** 此告警关联 风控 团队负责的 Fraud Prevention 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **Scene break** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Circuit Breaker metric reaches threshold Scene break, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【北美风控】场景熔断告警 (ALR-125)
