...
### 中文说明
此告警在 **Collection failed** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Availability 指标达到阈值 Collection failed，持续时间: Instant

**重要性:** 此告警关联 存储平台 团队负责的 Redis Cloud, LDAS 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **Collection failed** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Availability metric reaches threshold Collection failed, duration: Instant

**Importance:** This alert is associated with the Storage team's Redis Cloud, LDAS domain services and requires response within < 5 minutes.
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】Redis 实例采集失败请检查是否存活 (ALR-049)
