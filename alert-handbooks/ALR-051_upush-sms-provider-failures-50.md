...
### 中文说明
此告警在 **>50 failures in 5min** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** API 指标达到阈值 >50 failures in 5min，持续时间: 5m

**重要性:** 此告警关联 中台 团队负责的 UPush, MDM 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **>50 failures in 5min** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** API metric reaches threshold >50 failures in 5min, duration: 5m
...
```
1. 告警名称和ID / Alert name and ID: 【UPUSH】【北美】五分钟短信供应商调用失败超过阈值 50 (ALR-051)
