...
### 中文说明
此告警在 **Response >1500ms** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L2** 级别服务。

**触发条件:** Latency 指标达到阈值 Response >1500ms，持续时间: Instant

**重要性:** 此告警关联 监控平台 团队负责的 iZeus APM, Grafana 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **Response >1500ms** is met. This is a **P2** priority alert affecting **L2** level services.

**Trigger Condition:** Latency metric reaches threshold Response >1500ms, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【iZeus-策略10】-服务响应时间（ms）大于1500 (ALR-070)
