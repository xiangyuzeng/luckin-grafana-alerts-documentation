...
### 中文说明
此告警在 **Golden Flow Task Exception (Day)** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Pipeline 指标达到阈值 Golden Flow Task Exception (Day)，持续时间: N/A

**重要性:** 此告警关联 架构数据 团队负责的 DataLink, Chronus 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **Golden Flow Task Exception (Day)** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Pipeline metric reaches threshold Golden Flow Task Exception (Day), duration: N/A

**Importance:** This alert is associated with the Architecture Data team's DataLink, Chronus domain services and requires response within < 5 minutes.
...
```
1. 告警名称和ID / Alert name and ID: datalink黄金流程任务异常(白天) (ALR-006)
