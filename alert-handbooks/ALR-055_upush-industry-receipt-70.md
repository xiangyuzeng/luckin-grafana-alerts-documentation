...
### 中文说明
此告警在 **<70% industry receipt** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** Success Rate 指标达到阈值 <70% industry receipt，持续时间: Instant

**重要性:** 此告警关联 中台 团队负责的 UPush, MDM 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **<70% industry receipt** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** Success Rate metric reaches threshold <70% industry receipt, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【UPUSH】【北美】行业短信回执成功率低于 70% (ALR-055)
