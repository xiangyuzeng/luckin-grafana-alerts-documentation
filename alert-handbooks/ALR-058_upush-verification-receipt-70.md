...
|----------------|----------|
| **Alert ID** | ALR-058 |
| **告警名称 Alert Name** | 【UPUSH】【北美】验证码回执成功率低于 70% |
| **优先级 Priority** | P1 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | SMS-UPUSH |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Success Rate |
| **阈值/条件 Threshold** | <70% verification receipt |
...
### 中文说明
此告警在 **<70% verification receipt** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** Success Rate 指标达到阈值 <70% verification receipt，持续时间: Instant

**重要性:** 此告警关联 中台 团队负责的 UPush, MDM 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **<70% verification receipt** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** Success Rate metric reaches threshold <70% verification receipt, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【UPUSH】【北美】验证码回执成功率低于 70% (ALR-058)
