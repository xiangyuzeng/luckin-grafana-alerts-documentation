...
### 中文说明
此告警在 **>90% (Voice)** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** CPU 指标达到阈值 >90% (Voice)，持续时间: Instant

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **>90% (Voice)** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** CPU metric reaches threshold >90% (Voice), duration: Instant
...
1. 复杂聚合查询消耗过多CPU / Complex aggregation queries consuming excessive CPU
2. 大量并发搜索请求 / High concurrent search requests
3. 索引过程中的CPU峰值 / CPU spike during indexing
4. 分片不均衡导致热点节点 / Shard imbalance causing hot nodes
5. 正则表达式或脚本查询 / Regex or script queries
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】AWS-ES CPU 使用率大于90%_语音 (ALR-034)
