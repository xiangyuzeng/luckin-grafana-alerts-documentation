...
|----------------|----------|
| **Alert ID** | ALR-086 |
| **告警名称 Alert Name** | 【默认策略】异常okhttp总数大于等于50 |
| **优先级 Priority** | P2 |
| **服务等级 Service Level** | L2 - Normal Business/Infrastructure (普通业务服务/普通基础服务) |
| **类别 Category** | APM-Default |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | HTTP |
| **阈值/条件 Threshold** | >=50 okhttp exceptions |
...
### 中文说明
此告警在 **>=50 okhttp exceptions** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L2** 级别服务。

**触发条件:** HTTP 指标达到阈值 >=50 okhttp exceptions，持续时间: Instant

**重要性:** 此告警关联 运维开发 团队负责的 Auth, CI/CD, Monitoring 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **>=50 okhttp exceptions** is met. This is a **P2** priority alert affecting **L2** level services.

**Trigger Condition:** HTTP metric reaches threshold >=50 okhttp exceptions, duration: Instant

**Importance:** This alert is associated with the DevOps team's Auth, CI/CD, Monitoring domain services and requires response within < 30 minutes.
...
```
1. 告警名称和ID / Alert name and ID: 【默认策略】异常okhttp总数大于等于50 (ALR-086)
