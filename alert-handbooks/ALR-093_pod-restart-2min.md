...
|----------------|----------|
| **Alert ID** | ALR-093 |
| **告警名称 Alert Name** | 【pod-全局】P0 Pod 2m内发生重启请关注 |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | Pod |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Stability |
| **阈值/条件 Threshold** | Pod restarted in 2min |
| **持续时间 Duration** | 2m |
...
### 中文说明
此告警在 **Pod restarted in 2min** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Stability 指标达到阈值 Pod restarted in 2min，持续时间: 2m

**重要性:** 此告警关联 运维开发 团队负责的 Auth, CI/CD, Monitoring 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **Pod restarted in 2min** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Stability metric reaches threshold Pod restarted in 2min, duration: 2m
...
```
1. 告警名称和ID / Alert name and ID: 【pod-全局】P0 Pod 2m内发生重启请关注 (ALR-093)
