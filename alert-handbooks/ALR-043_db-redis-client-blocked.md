...
### 中文说明
此告警在 **Client blocked** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L0** 级别服务。

**触发条件:** Connectivity 指标达到阈值 Client blocked，持续时间: Instant

**重要性:** 此告警关联 存储平台 团队负责的 Redis Cloud, LDAS 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **Client blocked** is met. This is a **P1** priority alert affecting **L0** level services.

**Trigger Condition:** Connectivity metric reaches threshold Client blocked, duration: Instant
...
1. 客户端连接池配置不当 / Client connection pool misconfiguration
2. 网络延迟或丢包 / Network latency or packet loss
3. 阻塞操作(BLPOP等)超时 / Blocking operations (BLPOP, etc.) timeout
4. 最大连接数限制 / Maximum connection limit reached
5. 集群节点间通信问题 / Cluster inter-node communication issues
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】Redis 发生客户端堵塞 (ALR-043)
