...
|----------------|----------|
| **Alert ID** | ALR-025 |
| **告警名称 Alert Name** | 【DB告警】AWS RDS 慢查询数量持续三分钟大于300个 |
| **优先级 Priority** | P2 |
...
| **阈值/条件 Threshold** | >300 slow queries for 3min |
| **持续时间 Duration** | 3m |
...
### 中文说明
此告警在 **>300 slow queries for 3min** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L1** 级别服务。

**触发条件:** Performance 指标达到阈值 >300 slow queries for 3min，持续时间: 3m

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **>300 slow queries for 3min** is met. This is a **P2** priority alert affecting **L1** level services.

**Trigger Condition:** Performance metric reaches threshold >300 slow queries for 3min, duration: 3m
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】AWS RDS 慢查询数量持续三分钟大于300个 (ALR-025)
