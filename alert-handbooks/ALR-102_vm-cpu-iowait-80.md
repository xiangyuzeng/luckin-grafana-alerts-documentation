...
|----------------|----------|
| **Alert ID** | ALR-102 |
| **告警名称 Alert Name** | 【vm-cpu】P0 5分钟内服务CPU_iowait每秒的使用率大于80% |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | VM |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | CPU |
| **阈值/条件 Threshold** | >80% iowait in 5min |
| **持续时间 Duration** | 5m |
...
### 中文说明
此告警在 **>80% iowait in 5min** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** CPU 指标达到阈值 >80% iowait in 5min，持续时间: 5m

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **>80% iowait in 5min** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** CPU metric reaches threshold >80% iowait in 5min, duration: 5m
...
```
1. 告警名称和ID / Alert name and ID: 【vm-cpu】P0 5分钟内服务CPU_iowait每秒的使用率大于80% (ALR-102)
