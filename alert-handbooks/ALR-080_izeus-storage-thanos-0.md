# 【iZeus】Storage-Receiver2Thanos-Receiver-0
...
|----------------|----------|
| **Alert ID** | ALR-080 |
| **告警名称 Alert Name** | 【iZeus】Storage-Receiver2Thanos-Receiver-0 |
| **优先级 Priority** | P1 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | APM-iZeus |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Storage |
| **阈值/条件 Threshold** | Thanos Receiver Issue |
...
### 中文说明
此告警在 **Thanos Receiver Issue** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** Storage 指标达到阈值 Thanos Receiver Issue，持续时间: Instant

**重要性:** 此告警关联 监控平台 团队负责的 iZeus APM, Grafana 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **Thanos Receiver Issue** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** Storage metric reaches threshold Thanos Receiver Issue, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【iZeus】Storage-Receiver2Thanos-Receiver-0 (ALR-080)
