...
### 中文说明
此告警在 **Restart/Failover (Voice)** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Failover 指标达到阈值 Restart/Failover (Voice)，持续时间: Instant

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **Restart/Failover (Voice)** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Failover metric reaches threshold Restart/Failover (Voice), duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】AWS RDS 发生重启或者主从切换_语音 (ALR-024)
