...
|----------------|----------|
| **Alert ID** | ALR-052 |
| **告警名称 Alert Name** | 【UPUSH】【北美】五分钟短信供应商返回值失败超过阈值 200 |
| **优先级 Priority** | P1 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | SMS-UPUSH |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | API |
| **阈值/条件 Threshold** | >200 return failures in 5min |
| **持续时间 Duration** | 5m |
| **响应时间 Response Time** | HIGH (< 15 minutes) |
| **责任团队 Owner Team** | MiddlePlatform - Middle Platform (中台) |
...
### 中文说明
此告警在 **>200 return failures in 5min** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** API 指标达到阈值 >200 return failures in 5min，持续时间: 5m

**重要性:** 此告警关联 中台 团队负责的 UPush, MDM 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **>200 return failures in 5min** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** API metric reaches threshold >200 return failures in 5min, duration: 5m

**Importance:** This alert is associated with the Middle Platform team's UPush, MDM domain services and requires response within < 15 minutes.
...
|-----------------|------------|------------------|-----------|
| iupushsms | L1 | 短信推送服务 SMS Push | MiddlePlatform |
| iupushapp | L1 | APP推送服务 App Push | MiddlePlatform |
...
```bash
# 检查UPUSH服务状态 Check UPUSH service status
kubectl get pods -n upush -o wide
kubectl logs -n upush -l app=iupushsms --tail=100

# 检查短信供应商API状态 Check SMS provider API status
# 查看Grafana Dashboard: SMS Provider Metrics

# 检查消息队列状态 Check message queue status
# 通过AWS SQS Console查看 View via AWS SQS Console

# 检查数据库连接 Check database connections
mysql -h [UPUSH_RDS_ENDPOINT] -u admin -p -e "SELECT COUNT(*) FROM sms_send_log WHERE create_time > DATE_SUB(NOW(), INTERVAL 5 MINUTE);"
```
...
1. 短信供应商服务异常 / SMS provider service abnormal
2. API速率限制触发 / API rate limit triggered
3. 账户余额不足 / Account balance insufficient
4. 短信模板审核未通过 / SMS template review not passed
5. 网络连接问题 / Network connection issues
...
|---------------|--------------------------|---------------------------|
| 初次响应无法解决 / Initial response cannot resolve | 5m | L2 Support |
...
|-----------|-----------------|
| **主要负责团队 Primary Team** | Middle Platform (中台) |
| **On-Call** | middle-oncall@luckin.com |
| **Slack Channel** | #middle-platform-alerts |
...
```
1. 告警名称和ID / Alert name and ID: 【UPUSH】【北美】五分钟短信供应商返回值失败超过阈值 200 (ALR-052)
