...
### 中文说明
此告警在 **>2ms avg** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L1** 级别服务。

**触发条件:** Latency 指标达到阈值 >2ms avg，持续时间: Instant

**重要性:** 此告警关联 存储平台 团队负责的 Redis Cloud, LDAS 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **>2ms avg** is met. This is a **P2** priority alert affecting **L1** level services.

**Trigger Condition:** Latency metric reaches threshold >2ms avg, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】Redis 实例命令平均时延大于2ms (ALR-044)
