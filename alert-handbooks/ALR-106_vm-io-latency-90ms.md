...
|----------------|----------|
| **Alert ID** | ALR-106 |
| **告警名称 Alert Name** | 【vm-io】P0 服务io耗时大于90ms且同比超过20ms |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | VM |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | IO |
| **阈值/条件 Threshold** | >90ms io latency |
...
### 中文说明
此告警在 **>90ms io latency** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** IO 指标达到阈值 >90ms io latency，持续时间: Instant

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **>90ms io latency** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** IO metric reaches threshold >90ms io latency, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【vm-io】P0 服务io耗时大于90ms且同比超过20ms (ALR-106)
