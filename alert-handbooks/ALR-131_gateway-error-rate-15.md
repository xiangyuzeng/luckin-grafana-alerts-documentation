...
### 中文说明
此告警在 **>15% error rate** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Errors 指标达到阈值 >15% error rate，持续时间: Instant

**重要性:** 此告警关联 微服务 团队负责的 Nacos, Gateway, Sentinel 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **>15% error rate** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Errors metric reaches threshold >15% error rate, duration: Instant

**Importance:** This alert is associated with the Microservices team's Nacos, Gateway, Sentinel domain services and requires response within < 5 minutes.
...
```bash
# 检查网关服务状态 Check gateway service status
kubectl get pods -n gateway -o wide
kubectl logs -n gateway -l app=luckyapigateway --tail=100

# 检查网关错误率 Check gateway error rate
# 查看Grafana Dashboard: API Gateway Metrics

# 检查Sentinel状态 Check Sentinel status
kubectl get pods -n sentinel -o wide
kubectl logs -n sentinel -l app=isentineldashboard --tail=100
```
...
1. 后端服务不可用 / Backend service unavailable
2. 请求超时 / Request timeout
3. 限流规则触发 / Rate limiting rules triggered
4. 证书过期或配置错误 / Certificate expired or misconfigured
5. 路由配置错误 / Routing configuration error
...
|-----------|-----------------|
| **主要负责团队 Primary Team** | Microservices (微服务) |
| **On-Call** | microservice-oncall@luckin.com |
| **Slack Channel** | #microservice-alerts |
...
```
1. 告警名称和ID / Alert name and ID: 【网关告警】错误率大于15% (ALR-131)
