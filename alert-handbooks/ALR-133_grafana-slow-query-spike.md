# Slow Query Spike - High Rate Alert
...
|----------------|----------|
| **Alert ID** | ALR-133 |
| **告警名称 Alert Name** | Slow Query Spike - High Rate Alert |
| **优先级 Priority** | P2 |
...
| **阈值/条件 Threshold** | rate > 1/sec for 5min |
| **持续时间 Duration** | 5m |
...
### 中文说明
此告警在 **rate > 1/sec for 5min** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L1** 级别服务。

**触发条件:** Performance 指标达到阈值 rate > 1/sec for 5min，持续时间: 5m

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **rate > 1/sec for 5min** is met. This is a **P2** priority alert affecting **L1** level services.

**Trigger Condition:** Performance metric reaches threshold rate > 1/sec for 5min, duration: 5m
...
```
1. 告警名称和ID / Alert name and ID: Slow Query Spike - High Rate Alert (ALR-133)
