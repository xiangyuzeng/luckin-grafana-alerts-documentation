...
|----------------|----------|
| **Alert ID** | ALR-121 |
| **告警名称 Alert Name** | 【北美-业务告警】过去5分钟支付金额小于500分 |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | Business |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Payments |
| **阈值/条件 Threshold** | <500 cents in 5min |
| **持续时间 Duration** | 5m |
...
### 中文说明
此告警在 **<500 cents in 5min** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Payments 指标达到阈值 <500 cents in 5min，持续时间: 5m

**重要性:** 此告警关联 销售业务 团队负责的 Orders, Payments, CRM, Marketing 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **<500 cents in 5min** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Payments metric reaches threshold <500 cents in 5min, duration: 5m
...
```
1. 告警名称和ID / Alert name and ID: 【北美-业务告警】过去5分钟支付金额小于500分 (ALR-121)
