...
### 中文说明
此告警在 **>70% for 3min** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** CPU 指标达到阈值 >70% for 3min，持续时间: 3m

**重要性:** 此告警关联 运维开发 团队负责的 Auth, CI/CD, Monitoring 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **>70% for 3min** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** CPU metric reaches threshold >70% for 3min, duration: 3m
...
```
1. 告警名称和ID / Alert name and ID: 【pod-cpu】P0 CPU使用率连续3分钟大于70% (ALR-091)
