...
### 中文说明
此告警在 **<60% marketing receipt** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L1** 级别服务。

**触发条件:** Success Rate 指标达到阈值 <60% marketing receipt，持续时间: Instant

**重要性:** 此告警关联 中台 团队负责的 UPush, MDM 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **<60% marketing receipt** is met. This is a **P2** priority alert affecting **L1** level services.

**Trigger Condition:** Success Rate metric reaches threshold <60% marketing receipt, duration: Instant

**Importance:** This alert is associated with the Middle Platform team's UPush, MDM domain services and requires response within < 30 minutes.
...
```
1. 告警名称和ID / Alert name and ID: 【UPUSH】【北美】营销短信回执成功率低于 60% (ALR-053)
