...
|----------------|----------|
| **Alert ID** | ALR-117 |
| **告警名称 Alert Name** | 【北美-业务告警】取消订单持续5分钟大于1单 |
| **优先级 Priority** | P1 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | Business |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Orders |
| **阈值/条件 Threshold** | >1 cancelled order for 5min |
| **持续时间 Duration** | 5m |
...
### 中文说明
此告警在 **>1 cancelled order for 5min** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L0** 级别服务。

**触发条件:** Orders 指标达到阈值 >1 cancelled order for 5min，持续时间: 5m

**重要性:** 此告警关联 销售业务 团队负责的 Orders, Payments, CRM, Marketing 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **>1 cancelled order for 5min** is met. This is a **P1** priority alert affecting **L0** level services.

**Trigger Condition:** Orders metric reaches threshold >1 cancelled order for 5min, duration: 5m

**Importance:** This alert is associated with the Sales team's Orders, Payments, CRM, Marketing domain services and requires response within < 15 minutes.
...
1. 支付网关异常 / Payment gateway abnormal
2. 库存服务不可用 / Inventory service unavailable
3. 订单服务数据库连接问题 / Order service database connection issues
4. 促销活动导致流量激增 / Promotional campaign causing traffic surge
5. 门店系统同步延迟 / Store system sync delay
...
```
1. 告警名称和ID / Alert name and ID: 【北美-业务告警】取消订单持续5分钟大于1单 (ALR-117)
