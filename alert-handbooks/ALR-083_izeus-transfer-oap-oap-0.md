...
### 中文说明
此告警在 **OAP to OAP Issue** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** Transfer 指标达到阈值 OAP to OAP Issue，持续时间: Instant

**重要性:** 此告警关联 监控平台 团队负责的 iZeus APM, Grafana 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **OAP to OAP Issue** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** Transfer metric reaches threshold OAP to OAP Issue, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【iZeus】Transfer-OAP2OAP-0 (ALR-083)
