...
|----------------|----------|
| **Alert ID** | ALR-115 |
| **告警名称 Alert Name** | 【vm-网卡】P0 出方向在5分钟内每秒错误的数据包大于20个 |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | VM |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Network |
| **阈值/条件 Threshold** | >20 error packets/s out |
| **持续时间 Duration** | 5m |
...
### 中文说明
此告警在 **>20 error packets/s out** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Network 指标达到阈值 >20 error packets/s out，持续时间: 5m

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **>20 error packets/s out** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Network metric reaches threshold >20 error packets/s out, duration: 5m
...
```
1. 告警名称和ID / Alert name and ID: 【vm-网卡】P0 出方向在5分钟内每秒错误的数据包大于20个 (ALR-115)
