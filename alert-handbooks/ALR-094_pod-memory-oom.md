...
|----------------|----------|
| **Alert ID** | ALR-094 |
| **告警名称 Alert Name** | 【pod-宕机】P1 WSS内存使用率连续3分钟等于100%(OOM参考) |
| **优先级 Priority** | P1 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | Pod |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Memory |
| **阈值/条件 Threshold** | 100% memory for 3min |
| **持续时间 Duration** | 3m |
...
### 中文说明
此告警在 **100% memory for 3min** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L0** 级别服务。

**触发条件:** Memory 指标达到阈值 100% memory for 3min，持续时间: 3m

**重要性:** 此告警关联 运维开发 团队负责的 Auth, CI/CD, Monitoring 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **100% memory for 3min** is met. This is a **P1** priority alert affecting **L0** level services.

**Trigger Condition:** Memory metric reaches threshold 100% memory for 3min, duration: 3m
...
1. 内存泄漏 / Memory leak
2. JVM堆配置不当 / JVM heap misconfiguration
3. 缓存未设置上限 / Cache without upper limit
4. 大对象持续创建未释放 / Large objects created continuously without release
5. 容器内存限制设置过低 / Container memory limit set too low
...
```
1. 告警名称和ID / Alert name and ID: 【pod-宕机】P1 WSS内存使用率连续3分钟等于100%(OOM参考) (ALR-094)
