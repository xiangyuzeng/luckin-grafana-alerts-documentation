...
|----------------|----------|
| **Alert ID** | ALR-089 |
| **告警名称 Alert Name** | 【pod-cpu-兜底】P0 CPU使用率连续3分钟大于85% |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | Pod |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | CPU |
| **阈值/条件 Threshold** | >85% for 3min (Fallback) |
| **持续时间 Duration** | 3m |
...
### 中文说明
此告警在 **>85% for 3min (Fallback)** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** CPU 指标达到阈值 >85% for 3min (Fallback)，持续时间: 3m

**重要性:** 此告警关联 运维开发 团队负责的 Auth, CI/CD, Monitoring 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **>85% for 3min (Fallback)** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** CPU metric reaches threshold >85% for 3min (Fallback), duration: 3m

**Importance:** This alert is associated with the DevOps team's Auth, CI/CD, Monitoring domain services and requires response within < 5 minutes.
...
1. 应用程序代码效率低下 / Application code inefficiency
2. JVM Full GC频繁触发 / Frequent JVM Full GC triggers
3. 无限循环或递归调用 / Infinite loop or recursive calls
4. 资源限制(limits)设置过低 / Resource limits set too low
5. 外部依赖响应慢导致请求堆积 / Slow external dependency causing request accumulation
...
### CPU持续过高 / Sustained High CPU

**步骤 1:** 识别高CPU的Pod / Identify high-CPU pods: kubectl top pods -n [namespace]

**步骤 2:** 检查应用日志是否有异常 / Check application logs for anomalies

**步骤 3:** 分析JVM指标(如适用) / Analyze JVM metrics (if applicable)

**步骤 4:** 考虑水平扩展 / Consider horizontal scaling: kubectl scale deployment

**步骤 5:** 优化应用代码或增加CPU限制 / Optimize code or increase CPU limits
...
```
1. 告警名称和ID / Alert name and ID: 【pod-cpu-兜底】P0 CPU使用率连续3分钟大于85% (ALR-089)
