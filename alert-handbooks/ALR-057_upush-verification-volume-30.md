...
|----------------|----------|
| **Alert ID** | ALR-057 |
| **告警名称 Alert Name** | 【UPUSH】【北美】验证码发送量同比增加 30% |
| **优先级 Priority** | P2 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | SMS-UPUSH |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Volume |
| **阈值/条件 Threshold** | >30% YoY increase |
...
### 中文说明
此告警在 **>30% YoY increase** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L1** 级别服务。

**触发条件:** Volume 指标达到阈值 >30% YoY increase，持续时间: Instant

**重要性:** 此告警关联 中台 团队负责的 UPush, MDM 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **>30% YoY increase** is met. This is a **P2** priority alert affecting **L1** level services.

**Trigger Condition:** Volume metric reaches threshold >30% YoY increase, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【UPUSH】【北美】验证码发送量同比增加 30% (ALR-057)
