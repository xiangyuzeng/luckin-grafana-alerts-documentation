...
|----------------|----------|
| **Alert ID** | ALR-042 |
| **告警名称 Alert Name** | 【DB告警】Redis 内存使用率持续3分钟超过70% |
| **优先级 Priority** | P2 |
...
### 中文说明
此告警在 **>70% for 3min** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L1** 级别服务。

**触发条件:** Memory 指标达到阈值 >70% for 3min，持续时间: 3m

**重要性:** 此告警关联 存储平台 团队负责的 Redis Cloud, LDAS 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **>70% for 3min** is met. This is a **P2** priority alert affecting **L1** level services.

**Trigger Condition:** Memory metric reaches threshold >70% for 3min, duration: 3m
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】Redis 内存使用率持续3分钟超过70% (ALR-042)
