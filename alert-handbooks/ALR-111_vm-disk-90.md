...
|----------------|----------|
| **Alert ID** | ALR-111 |
| **告警名称 Alert Name** | 【vm-磁盘】P1 分区使用率大于90%请手动处理 |
| **优先级 Priority** | P1 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | VM |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Disk |
| **阈值/条件 Threshold** | >90% partition usage |
...
### 中文说明
此告警在 **>90% partition usage** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** Disk 指标达到阈值 >90% partition usage，持续时间: Instant

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **>90% partition usage** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** Disk metric reaches threshold >90% partition usage, duration: Instant

**Importance:** This alert is associated with the SRE team's Infrastructure, Networking domain services and requires response within < 15 minutes.
...
|-----------------|------------|------------------|-----------|
| All EC2 Hosts | L1 | 所有EC2主机 (158 instances) | SRE |
...
```bash
# 检查EC2实例状态 Check EC2 instance status
...
1. 日志文件未轮转 / Log files not rotated
2. 大文件未清理 / Large files not cleaned up
3. inode耗尽 / Inode exhaustion
4. 文件系统损坏 / Filesystem corruption
5. 临时文件积累 / Temporary file accumulation
...
**步骤 1:** 识别大文件 / Identify large files: du -sh /* | sort -h

**步骤 2:** 清理日志文件 / Clean log files: find /var/log -type f -mtime +7 -delete

**步骤 3:** 清理临时文件 / Clean temp files: rm -rf /tmp/*

**步骤 4:** 检查docker镜像占用 / Check docker images: docker system prune

**步骤 5:** 必要时扩展EBS卷 / Extend EBS volume if necessary
...
```
1. 告警名称和ID / Alert name and ID: 【vm-磁盘】P1 分区使用率大于90%请手动处理 (ALR-111)
...
- 实施日志轮转策略 / Implement log rotation policy
- 配置自动清理脚本 / Configure automatic cleanup scripts
- 监控磁盘使用趋势 / Monitor disk usage trends
- 定期进行系统健康检查 / Conduct regular system health checks
- 建立容量规划流程 / Establish capacity planning process
