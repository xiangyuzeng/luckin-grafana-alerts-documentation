...
|----------------|----------|
| **Alert ID** | ALR-090 |
| **告警名称 Alert Name** | 【pod-cpu】P0 CPU使用率连续10分钟大于50% |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | Pod |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | CPU |
| **阈值/条件 Threshold** | >50% for 10min |
| **持续时间 Duration** | 10m |
...
### 中文说明
此告警在 **>50% for 10min** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** CPU 指标达到阈值 >50% for 10min，持续时间: 10m

**重要性:** 此告警关联 运维开发 团队负责的 Auth, CI/CD, Monitoring 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **>50% for 10min** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** CPU metric reaches threshold >50% for 10min, duration: 10m
...
```
1. 告警名称和ID / Alert name and ID: 【pod-cpu】P0 CPU使用率连续10分钟大于50% (ALR-090)
