...
|----------------|----------|
| **Alert ID** | ALR-037 |
| **告警名称 Alert Name** | 【DB告警】AWS-ES 集群状态Yellow |
| **优先级 Priority** | P2 |
| **服务等级 Service Level** | L1 - Important Business/Infrastructure (重要业务服务/重要基础服务) |
| **类别 Category** | Database-ES |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Cluster Health |
| **阈值/条件 Threshold** | Status Yellow |
...
### 中文说明
此告警在 **Status Yellow** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L1** 级别服务。

**触发条件:** Cluster Health 指标达到阈值 Status Yellow，持续时间: Instant

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **Status Yellow** is met. This is a **P2** priority alert affecting **L1** level services.

**Trigger Condition:** Cluster Health metric reaches threshold Status Yellow, duration: Instant

**Importance:** This alert is associated with the DBA team's All Databases domain services and requires response within < 30 minutes.
...
|-----------------|------------|------------------|-----------|
| kbx | L2 | ES管理平台 ES Management | ArchitectureData |
| izeus | L2 | APM监控平台 APM Platform | CommonMonitor |

### 关联数据库 Affected Databases

- `luckyus_kbx (OpenSearch)`
- `luckyus_izeus (OpenSearch)`
...
```bash
# 检查OpenSearch域状态 Check OpenSearch domain status
aws opensearch describe-domains

# 检查集群健康 Check cluster health
...
# 检查分片分配 Check shard allocation
curl -X GET "https://[OPENSEARCH_ENDPOINT]/_cat/shards?v"

# 检查磁盘使用 Check disk usage
curl -X GET "https://[OPENSEARCH_ENDPOINT]/_cat/allocation?v"
```
...
1. 节点故障或离线 / Node failure or offline
2. 分片分配失败 / Shard allocation failure
3. 磁盘空间不足触发只读模式 / Disk space low triggering read-only mode
4. 网络分区导致集群分裂 / Network partition causing cluster split
5. 主分片无法恢复 / Primary shards unrecoverable
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】AWS-ES 集群状态Yellow (ALR-037)
...
- 定期清理旧索引数据 / Regularly clean old index data
- 优化分片策略 / Optimize sharding strategy
- 实施索引生命周期管理(ILM) / Implement Index Lifecycle Management (ILM)
- 监控并优化查询性能 / Monitor and optimize query performance
- 配置集群自动扩展 / Configure cluster auto-scaling
