...
### 中文说明
此告警在 **>3 exceptions/min** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L2** 级别服务。

**触发条件:** Errors 指标达到阈值 >3 exceptions/min，持续时间: 1m

**重要性:** 此告警关联 监控平台 团队负责的 iZeus APM, Grafana 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **>3 exceptions/min** is met. This is a **P2** priority alert affecting **L2** level services.

**Trigger Condition:** Errors metric reaches threshold >3 exceptions/min, duration: 1m
...
```
1. 告警名称和ID / Alert name and ID: 【iZeus-策略15】-服务每分钟异常数大于3 (ALR-073)
