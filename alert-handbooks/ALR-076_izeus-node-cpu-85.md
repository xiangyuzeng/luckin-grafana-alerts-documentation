...
### 中文说明
此告警在 **Node CPU >85%** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** CPU 指标达到阈值 Node CPU >85%，持续时间: Instant

**重要性:** 此告警关联 监控平台 团队负责的 iZeus APM, Grafana 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **Node CPU >85%** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** CPU metric reaches threshold Node CPU >85%, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【iZeus】Node-CPU-85 (ALR-076)
