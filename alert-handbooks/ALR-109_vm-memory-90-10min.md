...
### 中文说明
此告警在 **>90% for 10min** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L1** 级别服务。

**触发条件:** Memory 指标达到阈值 >90% for 10min，持续时间: 10m

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **>90% for 10min** is met. This is a **P1** priority alert affecting **L1** level services.

**Trigger Condition:** Memory metric reaches threshold >90% for 10min, duration: 10m
...
1. 进程内存泄漏 / Process memory leak
2. 文件缓存占用过多 / File cache consuming too much
3. Swap使用率过高 / High swap usage
4. 共享内存段未释放 / Shared memory segments not released
5. OOM Killer触发 / OOM Killer triggered
...
```
1. 告警名称和ID / Alert name and ID: 【vm-内存】P1 内存使用率大于90% 持续10分钟 (ALR-109)
