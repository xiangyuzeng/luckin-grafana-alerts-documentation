...
### 中文说明
此告警在 **>10% steal** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** CPU 指标达到阈值 >10% steal，持续时间: Instant

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **>10% steal** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** CPU metric reaches threshold >10% steal, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【vm-cpu】P0 服务CPU使用率窃取大于10% (ALR-103)
