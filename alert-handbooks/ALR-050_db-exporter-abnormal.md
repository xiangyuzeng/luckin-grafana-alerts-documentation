...
|----------------|----------|
| **Alert ID** | ALR-050 |
| **告警名称 Alert Name** | 【DB告警】exporter 进程异常 |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | Database-Exporter |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Process |
| **阈值/条件 Threshold** | Exporter process abnormal |
...
### 中文说明
此告警在 **Exporter process abnormal** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Process 指标达到阈值 Exporter process abnormal，持续时间: Instant

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **Exporter process abnormal** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Process metric reaches threshold Exporter process abnormal, duration: Instant
...
|-----------------|------------|------------------|-----------|
| All Database Exporters | L0 | 所有数据库监控导出器 | DBA |
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】exporter 进程异常 (ALR-050)
