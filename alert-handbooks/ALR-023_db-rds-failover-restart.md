...
|----------------|----------|
| **Alert ID** | ALR-023 |
| **告警名称 Alert Name** | 【DB告警】AWS RDS 发生重启或者主从切换 |
| **优先级 Priority** | P0 |
| **服务等级 Service Level** | L0 - Core Business Service (核心业务服务) |
| **类别 Category** | Database-RDS |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Failover |
| **阈值/条件 Threshold** | Restart/Failover detected |
...
### 中文说明
此告警在 **Restart/Failover detected** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Failover 指标达到阈值 Restart/Failover detected，持续时间: Instant

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **Restart/Failover detected** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Failover metric reaches threshold Restart/Failover detected, duration: Instant
...
1. 多可用区(Multi-AZ)故障转移事件 / Multi-AZ failover event
2. 主实例硬件故障 / Primary instance hardware failure
3. 手动触发的维护操作 / Manually triggered maintenance operation
4. AWS区域性基础设施问题 / AWS regional infrastructure issue
5. 网络分区导致的主从切换 / Network partition causing primary-replica switch
...
```
1. 告警名称和ID / Alert name and ID: 【DB告警】AWS RDS 发生重启或者主从切换 (ALR-023)
