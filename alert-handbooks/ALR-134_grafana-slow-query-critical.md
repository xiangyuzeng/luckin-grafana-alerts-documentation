# Slow Query Critical - Very High Rate Alert
...
### 中文说明
此告警在 **rate > 2/sec for 5min** 条件满足时触发。该告警属于 **P1** 优先级，影响 **L0** 级别服务。

**触发条件:** Performance 指标达到阈值 rate > 2/sec for 5min，持续时间: 5m

**重要性:** 此告警关联 数据库管理 团队负责的 All Databases 领域服务，需要在 < 15 minutes 内响应处理。

### English Description
This alert triggers when the condition **rate > 2/sec for 5min** is met. This is a **P1** priority alert affecting **L0** level services.

**Trigger Condition:** Performance metric reaches threshold rate > 2/sec for 5min, duration: 5m
...
```
1. 告警名称和ID / Alert name and ID: Slow Query Critical - Very High Rate Alert (ALR-134)
