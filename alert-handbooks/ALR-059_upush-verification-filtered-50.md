...
|----------------|----------|
| **Alert ID** | ALR-059 |
| **告警名称 Alert Name** | 【UPUSH】【北美】验证码过滤数超过 50 |
| **优先级 Priority** | P2 |
| **服务等级 Service Level** | L2 - Normal Business/Infrastructure (普通业务服务/普通基础服务) |
| **类别 Category** | SMS-UPUSH |
| **系统 System** | VMAlert |
| **指标类型 Metric Type** | Filter |
| **阈值/条件 Threshold** | >50 verification filtered |
...
### 中文说明
此告警在 **>50 verification filtered** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L2** 级别服务。

**触发条件:** Filter 指标达到阈值 >50 verification filtered，持续时间: Instant

**重要性:** 此告警关联 中台 团队负责的 UPush, MDM 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **>50 verification filtered** is met. This is a **P2** priority alert affecting **L2** level services.

**Trigger Condition:** Filter metric reaches threshold >50 verification filtered, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【UPUSH】【北美】验证码过滤数超过 50 (ALR-059)
