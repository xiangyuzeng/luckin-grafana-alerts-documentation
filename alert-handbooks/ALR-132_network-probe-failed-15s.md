...
### 中文说明
此告警在 **Probe failed for 15s** 条件满足时触发。该告警属于 **P0** 优先级，影响 **L0** 级别服务。

**触发条件:** Connectivity 指标达到阈值 Probe failed for 15s，持续时间: 15s

**重要性:** 此告警关联 SRE 团队负责的 Infrastructure, Networking 领域服务，需要在 < 5 minutes 内响应处理。

### English Description
This alert triggers when the condition **Probe failed for 15s** is met. This is a **P0** priority alert affecting **L0** level services.

**Trigger Condition:** Connectivity metric reaches threshold Probe failed for 15s, duration: 15s
...
|-----------------|------------|------------------|-----------|
| All Network Infrastructure | L0 | 所有网络基础设施 | SRE |
...
```bash
# 检查网络探测状态 Check network probe status
ping -c 5 [TARGET_IP]
traceroute [TARGET_IP]
mtr -r -c 10 [TARGET_IP]

# 检查VPN/专线状态 Check VPN/Direct Connect status
aws directconnect describe-connections
aws ec2 describe-vpn-connections

# 检查安全组规则 Check security group rules
aws ec2 describe-security-groups --group-ids [SG_ID]

# 检查网络接口状态 Check network interface status
...
|---------------|--------------------------|---------------------------|
| 初次响应无法解决 / Initial response cannot resolve | 15s | L2 Support |
...
```
1. 告警名称和ID / Alert name and ID: 【网络质量US-机房互拨】探测目标连续15s失败 (ALR-132)
