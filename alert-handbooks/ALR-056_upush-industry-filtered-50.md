...
### 中文说明
此告警在 **>50 industry filtered** 条件满足时触发。该告警属于 **P2** 优先级，影响 **L2** 级别服务。

**触发条件:** Filter 指标达到阈值 >50 industry filtered，持续时间: Instant

**重要性:** 此告警关联 中台 团队负责的 UPush, MDM 领域服务，需要在 < 30 minutes 内响应处理。

### English Description
This alert triggers when the condition **>50 industry filtered** is met. This is a **P2** priority alert affecting **L2** level services.

**Trigger Condition:** Filter metric reaches threshold >50 industry filtered, duration: Instant
...
```
1. 告警名称和ID / Alert name and ID: 【UPUSH】【北美】行业短信过滤数超过 50 (ALR-056)
